      ...
      tabItem(tabName = "my_forms",
              ...
              fluidRow(
                box(
                  ...
                  textInput("bank_born", label = "Fundação do Banco", placeholder = "Digite quando o banco foi fundado"),
                  ...
                  radioButtons("bank_cadastro", label = "Os cadastros são digitalizados?",choices = c("Sim" = "sim", "Não" = "não"),selected = NULL),
                  radioButtons("bank_cofre", label = "O banco possui um cofre?",choices = c("Sim" = "sim", "Não" = "não"),selected = NULL),
                  radioButtons("confirmação",label = "O seu banco utiliza a plataforma E-Dinheiro?",choices = c("Sim" = "sim", "Não" = "não"),selected = NULL),
                  textInput("bank_workers", label = "Número de funcionários", placeholder = "Digite o número de funcionários do seu Banco"),
                  
                  radioButtons("bank_coin",label = "O banco usa a moeda social/local de papel?",choices = c("Sim" = "sim", "Não" = "não"),selected = NULL),
                  ...
                  radioButtons("bank_corresponde",label = "O banco é um correspondente bancário?",choices = c("Sim" = "sim", "Não" = "não"),selected = NULL),
                  ...
                  radioButtons("bank_propaganda",label = "O banco utiliza de alguma estratégia de “propaganda”?",choices = c("Sim" = "sim", "Não" = "não"),selected = NULL),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_button, {
    ...
    req(
      ...
      input$bank_coin,
      input$bank_origin,
      input$bank_corresponde,
      input$bank_clientes,
      input$bank_empreendimentos,
      input$bank_linha_credito,
      input$bank_valor_credito,
      input$bank_tedence,
      input$bank_maior_cliente,
      input$bank_propaganda,
      input$bank_acesso_dados
    )
    
    # Verifica campos condicionais
    if (input$bank_coin == "Sim") req(input$bank_coin_imp)
    if (input$bank_origin == "Outros") req(input$bank_origin)
    if (input$bank_corresponde == "Sim" || input$bank_corresponde == "Não") req(input$bank_link)
    if (input$bank_linha_credito == "Outros") req(input$linha_credito_details)
    if (input$bank_propaganda == "sim") req(input$propaganda_details)
    ...
    # Verifica se o diretório .git existe e inicializa o repositório
    if (!dir.exists(".git")) {
    ...
    if (length(status) > 0) {
      # Adiciona e comita as alterações não comitadas
      system("git add .")
      system('git commit -m "Commitando alterações locais antes do pull"')
    }
    
    # Faz o pull para pegar as últimas alterações do repositório remoto
    system('git pull origin main --rebase')  # Usando rebase para evitar conflitos
    
    # Adiciona e comita o novo arquivo CSV

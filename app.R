...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_button, {
    ...
    # Verifica se o repositório Git existe e inicializa se necessário
    if (!dir.exists(".git")) {
      system('git init')
    ...
    # Adiciona as alterações locais antes de fazer o pull
    ...
    # Faz o pull e resolve conflitos automaticamente
    pull_result <- system('git pull origin main --rebase', intern = TRUE)
    if (any(grepl("CONFLICT", pull_result))) {
      # Força a priorização do `dados.csv` local
      system('git checkout --ours dados.csv')
